...
import { PLATFORMS, LANGUAGES, DIFFICULTIES } from "../constants/platforms";
import {
...
import type { Language, Platform, Problem, ProblemListRow } from "../types/domain";
import type { ScreeningListRow, ScreeningQuestion } from "../types/domain";
import type { ScreeningCategory, ScreeningTypeOption } from "../types/domain";

type HistoryTab = "problems" | "screening";

function stubPlatform(name: string): Platform {
  return {
    id: "unknown",
    name,
    color: "#888",
...
function stubLanguage(name: string): Language {
  return { id: "unknown", name, icon: "?", color: "#888" };
}

function stubScreeningCategory(name: string | undefined): ScreeningCategory {
  return {
    id: "unknown",
    name: name ?? "Unknown",
    color: "#888",
...
function stubScreeningType(id: string | undefined): ScreeningTypeOption {
  const safe = id ?? "concept";
  return {
    id: safe,
    name: safe,
    icon: "?",
...
export default function ProblemHistory({
  ...
  const [screenings, setScreenings] = useState<ScreeningListRow[]>([]);
  ...
  const handleCardClick = async (id) => {
    try {
      ...
      onViewProblem({
        ...
        platform:
          PLATFORMS.find((p) => p.name === data.platform) ??
          stubPlatform(data.platform),
        language:
  ...
  const handleScreeningCardClick = async (id) => {
    try {
      ...
      onViewScreening({
        question: q,
        category:
          SCREENING_CATEGORIES.find((c) => c.name === q.category) ??
          stubScreeningCategory(q.category),
        type:
          SCREENING_TYPES.find((t) => t.id === q.type) ??
          stubScreeningType(q.type),
      });
    } catch {
