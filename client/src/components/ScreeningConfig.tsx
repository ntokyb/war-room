...
import { generateScreening } from "../services/api";
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Generation failed. Check API key, server, and try again.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <main style={{ maxWidth: "820px", margin: "0 auto", padding: "32px 20px" }}>
      ...
      >
        <input
          type="checkbox"
          checked={forceNew}
          onChange={(ev) => setForceNew(ev.target.checked)}
          style={{ accentColor: category.color }}
        />
        New question (skip cache — calls Claude)
      </label>
