    ...
    <main style={{ maxWidth: '820px', margin: '0 auto', padding: '32px 20px' }}>
      ...
      >
        <div>
          ...
          >
            {platform.icon} {platform.name.toUpperCase()} · {language.icon} {language.name.toUpperCase()} ·{' '}
            <span style={{ color: dc }}>{problem.difficulty?.toUpperCase()}</span>
          </div>
          <div style={{ fontSize: '20px', fontWeight: 700, color: '#f0f0ff' }}>{problem.title}</div>
        </div>
        <div style={{ color: '#00cfff', fontSize: '18px', fontVariantNumeric: 'tabular-nums' }}>
          {timer.format()}
        </div>
      </div>
      ...
      {problem.constraints?.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ fontSize: '11px', color: '#444', letterSpacing: '2px', marginBottom: '10px' }}>
            CONSTRAINTS
          </div>
          <ul style={{ margin: 0, paddingLeft: '18px', color: '#999', fontSize: '13px', lineHeight: 1.8 }}>
      ...
      {showFull && (
        <div>
          <CollapsibleSection label="STEP 0 — IDE SETUP" icon="🖥" color="#00cfff">
            {problem.ideSetup?.map((step, i) => (
              ...
              >
                ...
                <div style={{ fontSize: '13px', color: '#aaa', lineHeight: 1.7 }}>{step}</div>
              </div>
            ))}
          </CollapsibleSection>

          <CollapsibleSection label="THINK BEFORE YOU TYPE" icon="🧠" color="#bf7fff">
            <div style={{ fontSize: '12px', color: '#555', marginBottom: '14px' }}>
              Strong engineers spend time clarifying inputs, outputs, and edge cases before typing.
            </div>
            {problem.thinkingProcess?.map((t, i) => (
              ...
              >
                <div style={{ color: '#bf7fff', fontSize: '14px', marginTop: '2px' }}>◈</div>
                <div style={{ fontSize: '13px', color: '#bbb', lineHeight: 1.7 }}>{t}</div>
              </div>
            ))}
          </CollapsibleSection>

          <CollapsibleSection label="STEP-BY-STEP SOLUTION" icon="⚡" color="#00ff88">
              ...
              <div key={i} style={{ marginBottom: '20px' }}>
                ...
                >
                  ...
                  <div style={{ textAlign: 'left', flex: 1 }}>
                    <div style={{ fontSize: '13px', color: '#e0e0f0', fontWeight: 600 }}>{s.title}</div>
                  </div>
                  <div style={{ color: '#444', fontSize: '14px' }}>{activeStep === i ? '▲' : '▼'}</div>
                </button>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
            ...
            >
              <div style={{ fontSize: '10px', color: '#555', letterSpacing: '2px', marginBottom: '6px' }}>
                TIME
              </div>
              <div style={{ fontSize: '13px', color: '#00cfff' }}>{problem.complexity?.time}</div>
            </div>
            <div
              style={{
                flex: 1,
            ...
            >
              ...
              <div style={{ fontSize: '13px', color: '#bf7fff' }}>{problem.complexity?.space}</div>
            </div>
          </div>
          ...
          <CollapsibleSection label="COMMON MISTAKES" icon="⚠" color="#ff5e7a">
            {problem.commonMistakes?.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: '12px', marginBottom: '10px' }}>
                ...
                <div style={{ fontSize: '13px', color: '#bbb', lineHeight: 1.7 }}>{m}</div>
              </div>
            ))}
          </CollapsibleSection>
          ...
          >
            <div style={{ fontSize: '10px', color: '#00ff8870', letterSpacing: '2px', marginBottom: '8px' }}>
              ⬡ SENIOR WISDOM
            </div>
            <div style={{ fontSize: '14px', color: '#00ff88', lineHeight: 1.8, fontStyle: 'italic' }}>
